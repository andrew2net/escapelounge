...
class UserGamesController < ApplicationController
  ...
  def step
    ...
    step_answers = @user_game.step_answers.where(
      game_step_solution_id: @step.game_step_solutions.ids
    )
    if step_answers.any?
      @answer   = step_answers.first.answer
      @answered = true
    end
    join = 'LEFT JOIN step_answers ON step_answers.game_step_solution_id='\
      "game_step_solutions.id AND step_answers.user_game_id=#{@user_game.id}"
    @questions = @step.game_step_solutions.select(
      'game_step_solutions.*, step_answers.answer'
    ).joins(join)
  end
  ...
  def answer
    ...
    if @user_game.finished_at
      render json: { result: 'finish',
                     redirect: user_game_result_url(@user_game) }
    else
      ...
      # check if the step is not empty (has solutions)
        ...
        if @step.multi_questions?
          ...
          if results
            params[:questions].each do |question_id, answer|
              sa = StepAnswer.find_or_initialize_by(
                user_game_id: @user_game.id, game_step_solution_id: question_id
              )
          ...
          solution = @step.game_step_solutions
                          .where('LOWER(solution) = ?', answ).first
          if solution
  ...
  def check_answer
    render json: {
      correct: GameStepSolution.check_answer(params[:id], params[:value])
    }
  end
  ...
  def result
    ...
    @pauses_duration = (@user_game.finished_at - @user_game.created_at)
                       .round - @user_game.result.to_i
  end
  ...
  def set_step
    ...
    @step = @user_game.game.allowed_step(step_id: params[:step_id],
                                         user_game_id: @user_game.id) ||
            @user_game.game.game_steps.first
  end
